...
function Library:CreateWindow(cfg)
        ...
        -- [FIX] Re-apply scale after a short delay — on some devices the camera
        -- viewport isn't ready at script start, so the first updateScale() uses
        -- the fallback 1920x1080 and the window appears too big until a resize.
        ...
        -- but the dragBar ends 80px before the right edge, so it never
        -- overlaps minBtn/closeBtn (which start at -74 and -38).
        dragBar.ZIndex = 6
        dragBar.Active = true
        ...
        function Window:CreateTab(name, icon)
                ...
                function tab:CreateToggle(tcfg)
                    ...
                    -- [FIX] Transparent overlay button — captures ALL taps on the
                    -- holder (including over sw/knob which have visible backgrounds
                    -- and would otherwise steal the input on mobile touch).
                ...
                function tab:CreateSlider(scfg)
                        ...
                        -- [FIX] Transparent overlay button covering the track row. Tapping the
                        -- fill or knob (children of track with visible backgrounds) would
                        -- otherwise steal the input and track.InputBegan would never fire.
                        local hit = Instance.new("TextButton")
                        hit.Size = UDim2.new(1, 0, 1, 0)
                        hit.BackgroundTransparency = 1
                        hit.Text = ""
