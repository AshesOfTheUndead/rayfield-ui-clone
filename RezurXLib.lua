...
function Library:CreateWindow(cfg)
        ...
        local function updateFit()
                ...
                local maxW = math.max(MIN_W, vp.X - 16)
                local maxH = math.max(MIN_H, vp.Y - 120)
                -- [FIX] Ensure max >= min before clamp. On very small screens
                -- maxW/maxH could fall below MIN_W/MIN_H, causing clamp to error.
